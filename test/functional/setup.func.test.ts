 ...
 */
import { encodeLeaseId, isbApi, normalizeLeaseId, saveState } from "./helpers";
...
describe("Setup", () => {
  it("creates a lease template", async () => {
    ...
    if (status !== 201) {
      throw new Error(
        `POST /leaseTemplates failed with ${status}: ${JSON.stringify(data)}`,
      );
    }
  ...
  it("leases an account", async () => {
    const { leaseTemplateId } = await import("./helpers").then((h) =>
      h.loadState(),
    );
    ...
    if (status !== 201) {
      throw new Error(
        `POST /leases failed with ${status}: ${JSON.stringify(data)}`,
      );
    }
    ...
    const leaseId = normalizeLeaseId(
      lease.leaseId ?? encodeLeaseId(lease.userEmail, lease.uuid),
    );
    ...
    saveState({
      ...
      leaseTerminationRequested: false,
      nukeCleanupVerified: false,
