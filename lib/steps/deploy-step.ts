...
export function createDeployStep(props: DeployStepProps): CodeBuildStep {
  ...
  const stackDeployCmd: Record<UpstreamStack, string> = {
    'account-pool': `npm run --workspace @amzn/innovation-sandbox-infrastructure cdk -- deploy InnovationSandbox-AccountPool --require-approval=never --role-arn arn:aws:iam::${props.targetAccount}:role/cdk-hnb659fds-cfn-exec-role-${props.targetAccount}-${props.targetRegion} --parameters ParentOuId=$PARENT_OU_ID --parameters HubAccountId=$HUB_ACCOUNT_ID --context deploymentMode=\${DEPLOYMENT_MODE:-STANDARD} --parameters IsbManagedRegions=$AWS_REGIONS`,
    idc: `npm run --workspace @amzn/innovation-sandbox-infrastructure cdk -- deploy InnovationSandbox-IDC --require-approval=never --role-arn arn:aws:iam::${props.targetAccount}:role/cdk-hnb659fds-cfn-exec-role-${props.targetAccount}-${props.targetRegion} --parameters IdentityStoreId=$IDENTITY_STORE_ID --parameters SsoInstanceArn=$SSO_INSTANCE_ARN --parameters OrgMgtAccountId=$ORG_MGT_ACCOUNT_ID --parameters HubAccountId=$HUB_ACCOUNT_ID --parameters AdminGroupName=\${ADMIN_GROUP_NAME:-InnovationSandboxAdmins} --parameters ManagerGroupName=\${MANAGER_GROUP_NAME:-InnovationSandboxManagers} --parameters UserGroupName=\${USER_GROUP_NAME:-InnovationSandboxUsers}`,
    data: `npm run --workspace @amzn/innovation-sandbox-infrastructure cdk -- deploy InnovationSandbox-Data --require-approval=never --role-arn arn:aws:iam::${props.targetAccount}:role/cdk-hnb659fds-cfn-exec-role-${props.targetAccount}-${props.targetRegion} --context deploymentMode=\${DEPLOYMENT_MODE:-STANDARD} --context nukeConfigFilePath=\${NUKE_CONFIG_FILE_PATH:-}`,
    compute: `npm run --workspace @amzn/innovation-sandbox-infrastructure cdk -- deploy InnovationSandbox-Compute --require-approval=never --role-arn arn:aws:iam::${props.targetAccount}:role/cdk-hnb659fds-cfn-exec-role-${props.targetAccount}-${props.targetRegion} --parameters OrgMgtAccountId=$ORG_MGT_ACCOUNT_ID --parameters IdcAccountId=$IDC_ACCOUNT_ID --parameters AcceptSolutionTermsOfUse=\${ACCEPT_SOLUTION_TERMS_OF_USE:-Accept} --context deploymentMode=\${DEPLOYMENT_MODE:-STANDARD} --context privateEcrRepo=\${PRIVATE_ECR_REPO:-}`,
  };
